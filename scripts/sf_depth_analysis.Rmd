...
#Removing unflagged day from KC10 where there is clearly bad data.
c_dm <- c_dm %>% 
  filter(!(site_id == "KC10" & date == "2021-08-05"))
...
#Selecting the dates with deep chl max to see what the composition was like
c_max_gt20 <- c_max %>% 
  filter(line_out_depth >= 20 & line_out_depth <= 30)

deep_max_qu39 <- c_max_gt20 %>% 
  filter(site_id == "QU39")

deep_max_qu39_list <- deep_max_qu39$date

deep_max_qu39_data <- c_dm %>% 
  filter(site_id == "QU39") %>% 
  filter(date %in% deep_max_qu39_list)
...
```
```{r}
#Want to look at how similar 0 and 5m depth are biomass wise.

c_bulk_wide <- c_bulk %>%
  select(date:line_out_depth, chla_dm) %>% 
  pivot_wider(names_from = line_out_depth, values_from = chla_dm) 

c_bulk_wide <- c_bulk_wide %>% 
  rename(m0 = `0`, m5 = `5`, m10 = `10`, m20 = `20`, m30 = `30`)

c_bulk_high <- c_bulk %>%
  filter(chla_dm > 100)

c_bulk_wide <- c_bulk_wide %>% 
  filter(m0 < 100 & m5 < 100)

ggscatter(c_bulk_wide, x = "m0", y = "m5", fill = "site_id", palette = "npg",
          add = "reg.line", size = 4, shape = 21) +
  geom_abline(intercept = 0, slope = 1) +
  facet_wrap(~ site_id) +
  xlim(0, 20) +
  ylim(0, 20) +
  stat_cor(label.y = 19.5, size = 10) +
  stat_regline_equation(label.y = 17, size = 10) +
  xlab("chl (0m, ug/L)") +
  ylab("chl (5m, ug/L)") +
  theme_bw() + 
  theme(legend.position = "none",
...
ggsave(here("figs_invent", "chl_bulk_0_5_scatter.png"), 
       width = 16, height = 6, dpi = 300)

```
```{r}
#Trying to look at comparability of bulk and SF
c_bulk_join <- c_bulk %>% 
  select(date, site_id, line_out_depth, bulk_dm = chla_dm)

c_dm_join <- c_dm %>% 
  select(date, site_id, line_out_depth, sum) %>% 
  distinct(date, site_id, line_out_depth, sum)

c_bulk_sf <- c_bulk_join %>% 
  left_join(c_dm_join)

c_bulk_sf <- c_bulk_sf %>% 
  filter(bulk_dm < 100 & sum < 100)

ggscatter(c_bulk_sf, x = "bulk_dm", y = "sum", fill = "site_id", palette = "npg",
          add = "reg.line", size = 4, shape = 21) +
  geom_abline(intercept = 0, slope = 1) +
  facet_wrap(~ site_id) +
  xlim(0, 30) +
  ylim(0, 30) +
  stat_cor(label.y = 29.5, size = 10) +
  stat_regline_equation(label.y = 27, size = 10) +
  xlab("chl (bulk, ug/L)") +
  ylab("chl (SF-Sum, ug/L)") +
  theme_bw() + 
  theme(legend.position = "none",
        text = element_text(size = 30))

ggsave(here("figs_invent", "chl_bulk_sfsum.png"), 
       width = 16, height = 6, dpi = 300)
