...
chl_2023_group_depth <- c_23_all %>% 
  filter(!is.na(filter_type)) %>% 
  group_by(group, line_out_depth) %>% 
  summarise(n_filt = n()) %>% 
  ungroup() 
...
fg_2 <- chl_2023_group_depth %>% 
  ggplot(aes(x = reorder(group, n_filt), y = n_filt, fill = as.factor(line_out_depth))) +
  geom_bar(stat = "identity", color = "black") +
  # geom_text(aes(x = group, y = n_tot, label = n_tot, fill = NULL),
  #             data = totals_2023, vjust = -0.5, size = 12) +
  # facet_wrap(~survey, scales = "free") +
   ylim(0, 1200) +
  scale_fill_brewer(palette = "RdYlBu", name = "Depth") +
  ylab("# of samples") +
  theme_bw() +
...
ggsave(here("figs_invent", "sample_numbers_group_depth.png"), fig_g,
       width = 16, height = 16, dpi = 300)
```

```{r}
chl_2023_calvert <- c_23_all %>% 
  filter(group == "Calvert OCGY" | group == "Quadra OCGY") %>%
  filter(!is.na(filter_type)) %>% 
...
f2 <- chl_2023_depth %>% 
  ggplot(aes(x = reorder(site_id, n_filt), y = n_filt, fill = as.factor(line_out_depth))) +
  geom_bar(stat = "identity", color = "black") +
  geom_text(aes(x = site_id, y = n_tot, label = n_tot, fill = NULL),
              data = totals_2023_calvert, vjust = -0.5, size = 8) +
  ylim(0, 1300) +
...
ggsave(here("figs_invent", "sample_numbers_calvert_quadra_2023_2.png"), fig,
       width = 8, height = 12, dpi = 300)
```

```{r}
qu39_sf_2dep <- c_23_all %>%
  filter(site_id == "QU39" & line_out_depth < 9 & !filter_type == "Bulk GF/F") 

qu39_bulk <- c_23_all %>%   
  filter(site_id == "QU39" & filter_type == "Bulk GF/F")

deep <- c_23_all %>%   
  filter(line_out_depth > 50 & site_id == "KC10")
```
...
```{r}
c_sf %>% 
  filter(line_out_depth < 50 & !filter_type == "2um") %>% 
  ggplot(aes(x = sf_sum, y = sf_perc*100, fill = as.factor(line_out_depth))) +
  geom_point(pch = 21, size = 6) + 
  geom_vline(xintercept = 5) +
  scale_fill_brewer(palette = "RdYlBu", name = "Depth") +
  facet_grid(filter_type ~ site_id) +
  labs(x = "Chla (ug/L)",
       y = "% Contribution") +
  theme_bw() +
...
ggsave(here("figs_invent", "scatter_chlorophyll_percent_size.png"),
       width = 16, height = 16, dpi = 300)
```
