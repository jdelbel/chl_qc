...
chl <- read_csv(here("files", "2024-04-12_HakaiData_chlorophyll.csv"))

#Upload HPLC data - entire QU39 dataset downloaded from the portal on 2022-0706
hplc <- read_csv(here("files", "2024-04-12_HakaiData_hplc.csv"))
...
hplc <- hplc %>% 
  ...
  select(date, site_id, collected, line_out_depth, all_chl_a)
```
...
chl_bulk %>%
  ...
  filter(is.na(chla_flag) & line_out_depth < 50 & year == 2023 & site_id == "QU39") %>% 
  ggplot(aes(x = date)) +
...
ggsave(here("figures_qc", "timeseries_qu39_bulk_tchla_2023.png"),
...
chl_bulk %>% 
  mutate(year = lubridate::year(date)) %>% 
  filter(is.na(chla_flag) & line_out_depth < 50 & year == 2023 & site_id == "QU39") %>%
  ggplot(aes(y = log1p(all_chl_a), x = log1p(chla))) +
  geom_point(aes(fill = as.factor(dilution_factor)), pch = 21, size = 5) +
...
#Should add HPLC and fluorescence to this and print for viewing.
ggsave(here("figures_qc", "scatter_no_flag_core_station_df_2023.png"),
...
chl_sf_qu39 <- chl %>% 
  ...
  mutate(n_filt = n()) %>% 
  ungroup() %>% 
  filter(n_samp == 3 & n_filt == 1) %>% 
  group_by(date, site_id, line_out_depth) %>% 
  mutate(sf_sum = sum(chla)) %>% 
  ungroup() %>% 
  select(date, year, site_id, line_out_depth, sf_sum) %>% 
  distinct()

chl_bulk_qu39 <- chl_bulk %>%
  mutate(year = lubridate::year(date)) %>%
  filter(site_id == "QU39" & year == 2023) %>%
  left_join(chl_sf_qu39)
...
fig_ad_sf <- chl_bulk_qu39 %>% 
  mutate(year = lubridate::year(date),
         analyzed_date = lubridate::date(analyzed)) %>% 
  filter(is.na(chla_flag) & line_out_depth < 50) %>%
  ggplot(aes(x = chla, y = sf_sum)) +
  geom_point(aes(fill = as.factor(analyzed_date)), pch = 21, size = 7) +
...
fig_month_sf <- chl_bulk_qu39 %>% 
  mutate(year = lubridate::year(date),
         month = lubridate::month(date),
         analyzed_date = lubridate::date(analyzed)) %>% 
  filter(is.na(chla_flag) & line_out_depth < 50) %>%
  ggplot(aes(x = chla, y = sf_sum)) +
  geom_point(aes(fill = as.factor(month)), pch = 21, size = 7) +
  geom_smooth(method = "lm", color = "black", formula = y~x, 
              fullrange = TRUE, se = F, size = 2) +
  scale_fill_brewer(palette = "RdYlBu") +
  stat_poly_eq(aes(label = paste(..eq.label.., ..rr.label.., sep = "~~")), 
...
fig_depth_sf <- chl_bulk_qu39 %>% 
  mutate(year = lubridate::year(date),
         month = lubridate::month(date),
         analyzed_date = lubridate::date(analyzed)) %>% 
  filter(is.na(chla_flag) & line_out_depth < 50) %>%
  ggplot(aes(x = chla, y = sf_sum)) +
  geom_point(aes(fill = as.factor(line_out_depth)), pch = 21, size = 7) +
  geom_smooth(method = "lm", color = "black", formula = y~x, 
              fullrange = TRUE, se = F, size = 2) +
  scale_fill_brewer(palette = "RdYlBu") +
  stat_poly_eq(aes(label = paste(..eq.label.., ..rr.label.., sep = "~~")), 
...
fig_sf_hplc <- fig_ad_hplc / fig_month_hplc / fig_depth_hplc

        # legend.position = c(0.79, 0.19))

#Should add HPLC and fluorescence to this and print for viewing.
ggsave(here("figures_qc", "scatter_tchla_bulk_2023.png"), fig_sf_hplc,
        width = 10, height = 16, dpi = 300)
```
...
#Should add HPLC and fluorescence to this and print for viewing.
ggsave(here("figures_qc", "scatter_no_flag_core_station_year.png"),
...
chl_bulk %>% 
  ...
  filter(is.na(chla_flag) & line_out_depth < 50) %>%
  ggplot(aes(x = all_chl_a, y = chla)) +
  geom_point(aes(fill = as.factor(line_out_depth)), pch = 21, size = 5) +
  geom_smooth(method = "lm", color = "black", formula = y~x-1, 
              fullrange = TRUE, se = F, size = 2) +
  scale_fill_brewer(palette = "RdYlBu") +
  stat_poly_eq(aes(label = paste(..eq.label.., ..rr.label.., sep = "~~")), 
...
#Should add HPLC and fluorescence to this and print for viewing.
ggsave(here("figures_qc", "scatter_no_flag_core_station_depth.png"),
        width = 14, height = 12, dpi = 300)
```
...
```{r}
distinct_analyzed <- chl %>% 
  mutate(date_analyzed = lubridate::date(analyzed)) %>% 
  distinct(date_analyzed)
```
