...
#Download hplc data
qu39_hplc <- read_xlsx(here("files", "hplc_qu39_2019_2020.xlsx"),
                      sheet = "Hakai Data")
...
chl <- read_csv(here("files", "qu39_kc10_dfo2_depth.csv"))
...
```
```{r}
#Selecting HPLC columns used in chemtax
qu39_hplc_chem <- qu39_hplc %>% 
  select(date, depth = line_out_depth, chl_c1_c2, peri, but_19 =`_19_but`,
         hex_19 = `_19_hex`, prasinoxanthin, alloxanthin, zeaxanthin, lutein,
         chl_b, all_chl_a)

#Pivoting longer 
qu39_hplc_tidy <- qu39_hplc_chem %>% 
  pivot_longer(c(chl_c1_c2:all_chl_a),
               names_to = "pigment", values_to = "conc")

qu39_all_chla <- qu39_hplc_tidy %>% 
  filter(pigment == "all_chl_a") %>% 
  select(date, depth, all_chla = conc)

qu39_hplc_tidy <- qu39_hplc_tidy %>% 
  left_join(qu39_all_chla)

qu39_hplc_tidy <- qu39_hplc_tidy %>% 
  mutate(all_chla_rat = conc/all_chla)
...
qu39 <- select(qu39,
               ...
               depth, Cyan = Cyanobacteria, Hapt = Hapto,
               Chlo = `Prasinophytes-3`, Cryp = Cryptophytes,
...
qu39_tidy <- qu39 %>% 
  pivot_longer(c(Cyan, Hapt, Chlo, Cryp, Dino, Dict, Diat),
...
order_chem <- c("Cyan", "Hapt", "Chlo", "Cryp",
                   "Dino", "Raph", "Dict", "Diat")

#Chemtax - Specify order of phyto groups for figures
qu39_tidy <- arrange(mutate(qu39_tidy,
                                phyto_group = factor(phyto_group,
                                levels = order_chem)))

qu39_tidy <- qu39_tidy %>% 
  arrange(date, depth, phyto_group)
```
...
f3 <- ggcorrplot(corr_green, outline.color = "white", lab = TRUE, type = "lower", 
  ...
  scale_fill_gradient(name = "R", limit = c(0.3, 1), low = "white", 
                      high =  "red") +
  annotate("text", x = 2, y = 3.4, label = "Chlo", size = 12) +
  theme_bw() +
  theme(plot.title = element_text(hjust = 0.5),
        axis.title.x = element_blank(),
        axis.text.x = element_blank(),
...
ggsave(here("figs_invent", "hplc_correlation.png"),
       width = 16, height = 16, dpi = 300)
```

```{r}
#Plotting out data ranges for 20 and 30m bulk chl.
f1 <- chl_bulk %>% 
  filter(line_out_depth == 20 & chla > 0) %>% 
  ggplot(aes(x = as.factor(month), y = chla, fill = site_id)) +
  ...
  geom_dotplot(aes(fill = site_id), color = "black", trim = FALSE, binaxis = 'y', 
               stackdir = 'center', dotsize = 0.2,
               position = position_dodge(0.8)) +
  geom_hline(yintercept = 1.0) +
  ggsci::scale_fill_npg() +
  scale_y_continuous(breaks=seq(0, 12, 2)) +
  annotate("text", x = 1, y = 14, label = "20m", size = 12) +
  labs(y = "chl (ug/L)",
       x = "Month") +
  theme_bw() +
...
f2 <- chl_bulk %>% 
  filter(line_out_depth == 30 & chla > 0) %>% 
  ggplot(aes(x = as.factor(month), y = chla, fill = site_id)) +
  geom_boxplot(position = position_dodge(preserve = "single"), alpha = 0.8,
               color = "black", lwd = 0.7) +
  geom_dotplot(aes(fill = site_id), color = "black", trim = FALSE, binaxis = 'y', 
  ...
  scale_y_continuous(breaks=seq(0, 12, 2)) +
  ggsci::scale_fill_npg() +
  annotate("text", x = 1, y = 11, label = "30m", size = 12) +
  labs(y = "chl (ug/L)",
       x = "Month") +
  theme_bw() +
...
ggsave(here("figs_invent", "bulk_chl_deep_conc.png"), fig,
       width = 16, height = 12, dpi = 300)

test <- chl_bulk %>% 
  filter(site_id == "QU39" & line_out_depth == 30 & month == 2)


```
```{r}

#Plotting out data for all depths
f1 <- chl_bulk %>% 
  filter(line_out_depth == 0 & chla > 0 & chla < 50) %>% 
  ggplot(aes(x = as.factor(month), y = chla, fill = site_id)) +
  geom_boxplot(position = position_dodge(preserve = "single"), alpha = 0.8,
               color = "black", lwd = 0.7) +
  geom_dotplot(aes(fill = site_id), trim = FALSE, binaxis = 'y', 
  ...
  ggsci::scale_fill_npg() +
  # scale_y_continuous(breaks=seq(0, 12, 2)) +
  annotate("text", x = 1, y = 26, label = "0m", size = 12) +
  labs(y = "chl (ug/L)",
       x = "Month") +
  theme_bw() +
  theme(legend.position = c(0.92, 0.80),
        legend.title = element_blank(),
        axis.title.x = element_blank(),
        axis.text.x = element_blank(),
...
f2 <- chl_bulk %>% 
  ...
  ggsci::scale_fill_npg() +
  # scale_y_continuous(breaks=seq(0, 12, 2)) +
  annotate("text", x = 1, y = 26, label = "5m", size = 12) +
  labs(y = "chl (ug/L)",
       x = "Month") +
  theme_bw() +
  theme(legend.position = "none",
...
f3 <- chl_bulk %>% 
  ...
  ggsci::scale_fill_npg() +
  # scale_y_continuous(breaks=seq(0, 12, 2)) +
  annotate("text", x = 1, y = 26, label = "10m", size = 12) +
  labs(y = "chl (ug/L)",
       x = "Month") +
  theme_bw() +
  theme(legend.position = "none",
...
ggsave(here("figs_invent", "bulk_chl_all_conc.png"), fig,
       width = 16, height = 20, dpi = 300)

```
```{r}
#Pigment:TChla ratios by depth

qu39_hplc_tidy %>% 
  filter(!pigment == "all_chl_a" & !depth == 30) %>% 
  ggplot(aes(x = as.factor(month), y = all_chla_rat, fill = as.factor(depth))) +
  geom_boxplot(position = position_dodge(preserve = "single"), alpha = 0.8,
               color = "black", lwd = 0.7) +
  geom_dotplot(aes(fill = depth), trim = FALSE, binaxis = 'y', 
               stackdir = 'center', dotsize = 0.3,
               position = position_dodge(0.8)) +
  facet_wrap(~ pigment, ncol = 1, scales = "free_y") +
  ggsci::scale_fill_npg() +
  # scale_y_continuous(breaks=seq(0, 12, 2)) +
  labs(y = "chl (ug/L)",
       x = "Month") +
...
ggsave(here("figs_invent", "test.png"),
       width = 16, height = 26, dpi = 300)

```
```{r}
#Working on nmds
chem_wide_grp <- qu39_tidy %>%
  select(date:TChla_mean) %>%
  pivot_wider(names_from = phyto_group, values_from = TChla_mean)

#Arranging according to site ID and date
chem_wide_grp <- chem_nmds %>%
  mutate(month = lubridate::month(date)) %>%
  arrange(site_id, date) %>%
  relocate(month, .after = date)
 
chem_conc_tr <- chem_wide_grp[, 5:ncol(chem_wide_grp)]

chem_conc_tr <- log10(chem_conc_tr + 1)

chem_perc_tr <- chem_wide_grp[, 5:ncol(chem_wide_grp)]

chem_perc_tr <- decostand(chem_perc_tr, method = "total")

chem_perc_tr <- sqrt(chem_perc_tr)

```

```{r}
#Run nMDS - needed 3 dimensions - no convergence with 2. Not sure what this means, but converged at 0.092 
chem_nmds <-  metaMDS(chem_conc_tr, distance = "bray", autotransform = FALSE,
                         k = 3, trymax = 100)

#Run nMDS on relative data - converged at 0.16
chem_nmds_rel <-  metaMDS(chem_perc_tr, distance = "bray", autotransform = FALSE,
                         k = 2, trymax = 100)

#Run with 3 dimensions - no convergence with 2. Not sure what this means, but converged at 0.092 

#Plotting stress
stressplot(chem_nmds)
stressplot(chem_nmds_rel)
```



```{r}
#Pulling out months and depths so that I can add to nmds plot
data.scores = as.data.frame(scores(chem_nmds))
data.scores$month = chem_wide_grp$month
data.scores$depth = chem_wide_grp$depth

data.scores_rel = as.data.frame(scores(chem_nmds_rel))
data.scores_rel$month = chem_wide_grp$month
data.scores_rel$depth = chem_wide_grp$depth
