...
```{r}
#Trying to see which filters have less than 3 size-fractions

#Converting the chlorophyll datasheet to long format.
c_long <- c %>% 
...
#so this is using the erddap download and the following is using the portal download. Why are the numbers different?
c_filt_check <- c_long %>%
  filter(!is.na(chla) & !filter_type == "Bulk GF/F") %>% 
  group_by(time, site_id, line_out_depth) %>%
  mutate(n_filt = n()) %>%
  ungroup() %>% 
  filter(n_filt < 3)

#Data downloaded from the portal using parameters being included in erddap query. Maybe it's catching data outside of the erddap query?
cp_filt_check <- c_p %>% 
...
#Pulling out important columns for merge with erdapp datasheet to join and try to cross compare.
...
#For some reason, the line out depth is in character format and needs to be converted to numeric to merge/join
test_e[ ,3] <- sapply(test_e[ ,3], as.numeric)

#So these filters are on the 
filt_check_join <- test_p %>% 
  left_join(test_e) %>% 
  filter(is.na(count_e))

filt_check_list <- filt_check_join %>% 
  select(hakai_id)
