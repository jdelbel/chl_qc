...
chl <- read_csv(here("files", "calvert_2022-04-20_chl.csv"))

#Upload HPLC data
hplc <- read_csv(here("files", "qcs01-2022-04-20_hplc.csv"))

#Upload CTD data
ctd <- read_excel(here("files", "calvert_2022-04-20_ctd.xlsx"),
                      sheet = "Data")

#Upload CTD metadata from drops sheet in portal excel download
ctd_meta <- read_excel(here("files",  "calvert_2022-04-20_ctd.xlsx"),
                      sheet = "Drops")
...
#Pulling out duplicates - 1 duplicate. Looks like filter type input incorrectly.
dup <- chl %>% 
  filter(rep > 1)
...
ctd <- ctd %>%
  ...
  select(castpk = `Cast PK`, ctdNum = `CTD serial number`, 
         ctdFirm = `CTD firmware version`, site_id = Station.x, lat = Latitude,
         long = Longitude, time = `Measurement time`, date, year,
         dep = `Depth (m)`, pres = `Pressure (dbar)`,
         flu = `Fluorometry Chlorophyll (ug/L)`)

#checking if there are duplicated CTD cast - There was a duplicate cast at KC10 on the most recent survey
check_ctd_dup <- ctd %>% 
  distinct(castpk, date, site_id) %>% 
  group_by(date, site_id) %>% 
  summarise(n_cast = n()) %>% 
  ungroup()

#checking min/max ctd cast depths for each castPK - All start at 1 except 2
check_ctd_depth <- ctd %>% 
  group_by(castpk) %>% 
...
#Depth check - where were samples collected - only bulk collected >30m.
check_depth <- chl %>% 
  distinct(line_out_depth, filter_type)

#Looking at calibrations applied to data - three calibrations
check_cali <- chl %>% 
  distinct(calibration, acid_ratio_correction_factor, acid_coefficient,
           calibration_slope)

#Looking for negative values - 5 records.
check_neg <- chl %>% 
  filter(chla < 0 | phaeo < 0)

#Looking for no concentration - lots, mostly new data not analyzed yet, but some older.
check_na <- chl %>% 
  filter(is.na(chla) | is.na(phaeo))

#Checking if there are any pre-existing flags - some are already flagged from previous rounds 
check_flag <- chl %>% 
  filter(!is.na(chla_flag) | !is.na(phaeo_flag))

check_flag_summary <- chl %>% 
  group_by(chla_flag) %>% 
  summarise(num_flag = n())
...
#Looking for filters with low acid ratios - lots. Need to look into these.
check_ar <- chl %>% 
...
check_ar %>% 
  ...
  facet_grid(chla_flag ~ site_id) +
  labs(x = "depth",
       y = "count") +
  theme_bw()

#So it looks like a lot of the unflagged data has low acid ratios at deeper depths.
```


```{r}
#For now, I want to filter out any data with bad flags, negative values
chl <- chl %>% 
  filter(chla > 0 & !is.na(chla)) %>% 
  filter(chla > 0 & chla_flag == "AV" | chla_flag == "ADL" | is.na(chla_flag))
```
...
#Looking for size fraction sets that have less than 3 filters - 19 records - 1 set AV flags.
check_sf_low <- chl_sf %>% 
  filter(filt_num < 3)
...
#Querying replicated filter types within a set - capturing where 3 filters, but two are replicated. 
#One sample with two 20 um filters
check_sf_dup <- chl_sf %>% 
  group_by(collected, site_id, line_out_depth, filter_type) %>% 
...
#Subsetting to remove the sample with duplicated filter
chl_sf <- chl_sf %>% 
  group_by(collected, site_id, line_out_depth, filter_type) %>% 
  mutate(filt_unique = n()) %>% 
  ungroup() %>% 
...
#Since I removed these, I now need to recount all the filters for each sample as there will be some with < 3
chl_sf <- chl_sf %>% 
  group_by(collected, site_id, line_out_depth) %>% 
  mutate(filt_num2 = n()) %>% 
  ungroup() %>% 
...
chl_comp <- chl_bulk %>%
  select(date, collected, site_id, line_out_depth, bulk_chla = chla) %>% 
...
ctd_join <- ctd %>% 
  select(date, site_id, ctdNum, line_out_depth = pres, flu) %>% 
...
chl_comp %>% 
  ...
  ggpubr::stat_cor(aes(label = ..rr.label..), label.y = 23, size = 8) +
  ggpubr::stat_regline_equation(label.y = 25, size = 8) +
  labs(x = "Bulk Chla (ug/L)",
       y = "SF-Sum Chla (ug/L)",
       fill = "Depth") +
  xlim(0, 30) + #Make sure to check data range and change these limits to include all data
  ylim(0, 30) +
  theme_bw()

#Looks good, with exception to two filters where there was clearly issues. 
```

```{r}
#Plotting scatter of bulk versus SF sum concentrations - coloring by station

chl_comp %>% 
  ggplot(aes(x = bulk_chla, y = sf_sum)) + 
  geom_point(aes(fill = as.factor(site_id)), pch = 21, size = 3) +
  geom_abline(slope = 1, intercept = 0) +
  geom_smooth(method = "lm", color = "black") +
  ggpubr::stat_cor(aes(label = ..rr.label..), label.y = 23, size = 8) +
  ggpubr::stat_regline_equation(label.y = 25, size = 8) +
