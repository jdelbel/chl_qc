...
chl <- read_csv(here("files", "qu39_2022-07-06.csv"))
...
#Pulling out duplicates - alot. Some have different Hakai IDs, but have same metadata with one being analyzed and the other not - I think DFO crosse comparison samples.
check_dup <- chl %>% 
  filter(rep > 1)

#these are the samples with duplicate hakai IDs. These are the bigger problem
check_dup_id <- chl %>% 
  filter(rep_id > 1)
...
#Looking at calibrations applied to data - doesn't look like recent calibrations have been applied - what's the newest data uploaded -  it should have the new information
check_cali <- chl %>% 
  distinct(calibration, acid_ratio_correction_factor, acid_coefficient,
           calibration_slope)

#Looking for negative values - lots - most are flagged already. Some interesting things to look for here.
check_neg <- chl %>% 
  filter(chla < 0 | phaeo < 0)

#There are lots and I need to investigate this.
check_na <- chl %>% 
...
#Need to look into these samples - No bulks until 10m. Some 20um at 5 and 10m which is weird.
```
```{r}
...
```{r}
#Pulling out all samples that haven't been QC'd -  for now removing samples that were not analyzed post 2022 on the new module. 
chl_qc <- chl %>% 
  filter(!is.na(analyzed) & analyzed < "2022-01-01" & 
...
#So lots of duplicates where multiple programs went out on a single day, but only one sample that is an exact replicate/duplicate
...
#Very odd trend as it would be expected that samples that weren't diluted would saturate the fluorometer and result in lower chla per HPLC TChla. Here the values are higher, although the trend line is <1, so higher values would go below 1:1 line and underpredict.
...
#Of the data that hasn't been QC'd, there are a few concerning points of very low chla for high TChla.

```

```{r}

#Plotting only un-QC'd data and coloring by whether the acid ratio is <1.2 - log10 scale
chl_bulk %>% 
  filter(is.na(chla_flag)) %>% 
...
```
```{r}
#I want to isolate these two samples with low ARs
check_bulk_ar_2 <- chl_bulk %>% 
  filter(is.na(chla_flag) & !is.na(all_chl_a) & acid_ratio < 1.2 &
           line_out_depth < 50)

#These are the two points showing up in the above plot.
```
...
ggsave(here("figures_qc", "test.png"),
        width = 16, height = 12, dpi = 300)
...
```{r}
#Looking at samples that were collected in the surface waters and have low acid ratios.
...
#Maybe one sample here that should be checked for havin low bulk concentration
...
#Definitely some disperson here, but doesn't seem to be more than expected
...
#there is a single 5m point where HPLC shows an early bloom, but there is not corresponding increase in any of the other variables.
