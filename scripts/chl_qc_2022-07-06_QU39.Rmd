...
chl_sf_sum_qcd <- chl_sf %>% 
  ...
  select(collected, line_out_depth, sf_sum, flag)

chl_sf_sum_raw <- chl_sf %>% 
  filter(is.na(chla_flag)) %>% 
  group_by(collected, line_out_depth) %>% 
  mutate(sf_sum = sum(chla),
         flag = "raw") %>% 
...
chl_sf_sum_join <- rbind(chl_sf_sum_qcd, chl_sf_sum_raw)
...
chl_comp <- chl_bulk %>%
  select(date, collected, line_out_depth, bulk_chla = chla, all_chl_a) %>% 
  filter(line_out_depth < 99) %>% 
  left_join(chl_sf_sum_join)
...
test <- chl_comp %>% 
  filter(!is.na(sf_sum)) %>% 
  mutate(diff = bulk_chla - sf_sum)

test_2 <- chl_qc %>% 
  filter(date == "2018-03-20" & line_out_depth == 10)
...
chl_comp %>% 
  ggplot(aes(x = bulk_chla, y = sf_sum, fill = flag)) +
  geom_point(pch = 21, size = 3.0) +
  geom_smooth(aes(color = flag), method = 'lm') +
  geom_abline(intercept = 0, slope = 1) +
  # xlim(0,25) +
  # ylim(0,25) +
  theme_bw() +
...
```{r}
#Looking at bulk data - box plot - sort of cool, but hard to see. Maybe if I print it would be better.
chl_bulk %>%
  filter(line_out_depth < 50) %>% 
  ggplot(aes(x = as.factor(month), y = chla)) +
  geom_boxplot() +
  geom_jitter(aes(fill = chla_flag), pch = 21) +
  facet_grid(line_out_depth ~ .)
